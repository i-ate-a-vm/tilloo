...
// Define SES config, which pulls from env vars or configmap
...
SES.prototype.notify = async function notify(message, lastErrorTime, failureCount, job) {
    if (!message.manualStop) {
        ...
        try {
            ...
            debug(`SES response: ${JSON.stringify(response)}`); // Debug statement after the sendEmail call
            debug(`SES messageId: ${response.MessageId}`); // Debug message id
